...
describe("Cart routes", () => {
  describe("Checkout", () => {
    ...
    it("should return 400 if cart is empty", async () => {
      ...
      .set("Authorization", `Bearer ${userOneAccessToken}`)//Fixed
      .send();
    ...
    it("should return 400 if user's address is not set", async () => {
      ...
      const res = await request(app)
      .put("/v1/cart/checkout")
        .set("Authorization", `Bearer ${userTwoAccessToken}`)//Fixed
        .send();
    ...
    it("should return 204 if cart is valid", async () => {
      ...
      const updatedCart = await Cart.findOne({ email: userOne.email });
